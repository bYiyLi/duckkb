...
* 统一语言：一律使用简体中文。
* 覆盖范围：所有公共模块、类、函数（async def）必须写 Docstring。
* 风格标准：遵循 Google Style (必须包含 Args 和 Returns)。
* 注释逻辑：只写 “为什么” (决策与陷阱)，不写 “是什么” (废话注释)。
* 待办标记：统一格式为 # TODO(name): 描述。
* 自动化检查：Docstring 缺失或格式错误由 ruff 强制拦截，不合规不准合并。
